-- 069: live-process lease survival + company-role recovery renew (founder 2026-08-13).
-- A live Company-role process is not refused just because a 120s lease lapsed.
-- Recovery heartbeat UPDATE must accept a consumed intent; admit already consumed it.
-- Scribe validator is NOT replaced here. DDL 041 is still held; Scribe rows have
-- no prior_lease_expires_at. Do not install the 041 body onto live Scribe.
-- Does not rewrite 040 bytes. CREATE OR REPLACE of the role validator only.
