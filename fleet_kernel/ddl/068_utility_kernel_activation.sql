-- 068: Utility Kernel activation graft (founder 2026-08-13).
-- Additive only. Does not retire the Company manifest or suspend workers.
...
BEGIN
    ...
    END IF;
    SELECT status, fleet_type, tenant_id
      INTO manifest_status, manifest_fleet, manifest_tenant
    ...
    END IF;
    IF TG_OP = 'INSERT'
       AND NEW.role_type = 'utility'
       AND NEW.fleet_type = 'company'
       AND NEW.tenant_id = '__CC_SUITE_COMPANY_ID__'
       AND manifest_status = 'active'
       AND manifest_fleet = 'company'
       AND manifest_tenant = '__CC_SUITE_COMPANY_ID__'
       AND NOT EXISTS (
            SELECT 1
              FROM kernel_tenant_fleet_manifest_roles
             WHERE manifest_id = NEW.manifest_id
               AND role_type = 'utility'
       ) THEN
        RETURN NEW;
    END IF;
    ...
    IF NEW.artifact->>'schema'='mise.cc-suite-activation-mandate.v1' THEN
        ...
        IF jsonb_typeof(NEW.artifact->'rollback_policy')<>'object'
           ...
           OR EXISTS (
            ...
            RAISE EXCEPTION 'founder activation mandate artifact is incomplete';
        END IF;
    ELSIF NEW.artifact->>'schema'='mise.utility-activation-mandate.v1' THEN
        IF NEW.status<>'active' OR NEW.closed_at IS NOT NULL
           OR NEW.mandate_id
              <>'__CC_SUITE_UTILITY_MANDATE_ID__'
           OR NEW.artifact->>'mandate_id'<>NEW.mandate_id
           OR NEW.artifact_sha256
              <>'__CC_SUITE_UTILITY_MANDATE_ARTIFACT_SHA256__'
           OR encode(digest(NEW.artifact_bytes,'sha256'),'hex')
              <>NEW.artifact_sha256
           OR convert_from(NEW.artifact_bytes,'UTF8')::JSONB
              IS DISTINCT FROM NEW.artifact
           ...
           OR NEW.artifact->>'company_id'<>'__CC_SUITE_COMPANY_ID__'
           OR NEW.artifact->>'host_kind'<>'macbook'
           OR jsonb_typeof(NEW.artifact->'ordered_roles')<>'array'
           OR jsonb_array_length(NEW.artifact->'ordered_roles')<>1
           OR NEW.artifact#>>'{ordered_roles,0,role_type}'<>'utility'
           OR NEW.artifact#>>'{ordered_roles,0,deployment_id}'<>'__CC_SUITE_UTILITY_DEPLOYMENT_ID__'
           OR NEW.artifact#>>'{ordered_roles,0,worker_id}'
              <>'__CC_SUITE_UTILITY_WORKER_ID__' THEN
            RAISE EXCEPTION 'Utility activation mandate artifact is incomplete';
        END IF;
    ELSE
        IF NEW.status<>'active' OR NEW.closed_at IS NOT NULL
